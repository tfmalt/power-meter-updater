...
up.on('error', (info) => {
  log.error('got error from:', info.location, info.error.message);
  log.error(info.error);
});

up.on('stored_data', (info) => {
  delete info.data.timestamp;
  delete info.data.time;

  log.info(info.list, info.data);
});
