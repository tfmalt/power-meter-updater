...
class UpdateMeter extends EventEmitter {
  ...
  start() {
    ...
    if (typeof process.env.POWER_SET_TIME !== 'undefined') {
      ...
      this.emit('power_set_time', timer.clock);
    }
  ...
  handleMinute() {
    ...
    return this.db.lrangeAsync('seconds', -60, -1)
      ...
      .then(values => {
        ...
        data.max     = values.length ? max(values) : 0;
        data.min     = values.length ? min(values) : 0;
        data.average = values.length ? parseFloat(data.count / values.length) : 0;
      ...
      .then(data => {
        ...
        this.emit('stored_data', {list: 'minutes', data});
        return data;
      })
      .catch(error => {
        this.emit('error', {location: 'handleMinute', error});
      });
  }
